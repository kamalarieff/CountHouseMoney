...
public class CropImageView extends FrameLayout {
    ...
    public CropImageView(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, com.edmodo.cropper.R.styleable.CropImageView, 0, 0);

        try {
            mGuidelines = ta.getInteger(com.edmodo.cropper.R.styleable.CropImageView_guidelines, DEFAULT_GUIDELINES);
            mFixAspectRatio = ta.getBoolean(com.edmodo.cropper.R.styleable.CropImageView_fixAspectRatio,
                    DEFAULT_FIXED_ASPECT_RATIO);
            mAspectRatioX = ta.getInteger(com.edmodo.cropper.R.styleable.CropImageView_aspectRatioX, DEFAULT_ASPECT_RATIO_X);
            mAspectRatioY = ta.getInteger(com.edmodo.cropper.R.styleable.CropImageView_aspectRatioY, DEFAULT_ASPECT_RATIO_Y);
            mImageResource = ta.getResourceId(com.edmodo.cropper.R.styleable.CropImageView_imageResource, DEFAULT_IMAGE_RESOURCE);
        } finally {
            ta.recycle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mBitmap != null) {
            ...
            // Otherwise, the picture is within frame banner_image bounds. Desired
            // width is
            // simply picture size
            else {
    ...
    public void rotateImage(int degrees) {
        ...
        Bitmap bitmapToBeRotate = mBitmap;
        mBitmap = Bitmap.createBitmap(bitmapToBeRotate, 0, 0, mBitmap.getWidth(), mBitmap.getHeight(), matrix, true);
        setImageBitmap(mBitmap);
        if (bitmapToBeRotate != mBitmap) {
            bitmapToBeRotate.recycle();
        }
